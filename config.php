<?PHP

$siteTitle = 'Celestia 12.0'; /* will be displayed above the url-bar */
$siteName = 'Celestia 12.0'; /* The biggest title on your homepage */
$siteTitleHome = 'Cutting Knots between Knowledge, Freedom and $$$'; // will be displayed above the url-bar when the home-page is open
$siteFooter = '© Team Celestia';
$siteDescription = 'An Exclusive Technical Talent Hunt dedicated to Open Source Community'; // fill in a description of your site, max 150 chars
$siteKeywords = 'celestia, celestia 12.0, symposium, ams college of engineering, aalim muhammad salegh college of engineering'; //keywords of your site,max 10 keywords, seperated by commas

$pageLink = array();
$pageLink['Home'] = 'home.php';
$pageLink['Testing slides'] = 'testslides.html';
$pageLink['Other content'] = 'othercontent.php';
$pageLink['Technical Events'] = 'techevents.php';
$pageLink['Non-Technical Events'] = 'nontechevents.php';
$pageLink['Paper Presentation'] = 'techevents.php#topcoders';
$pageLink['Online Events'] = 'onlinevents.php';





/*BACKGROUND OPTIONS*/
$enableBackgroundImg = false; 
$backgroundImg = 'img/bg/green.png'; 
$backgroundColor =  '#310C31';//'#260930'; 


$adminLogin = 'yourusername';
$adminPassword = 'yourpassword';

/*COMPRESSING */
$enableCompressionCss = false; // compress and combine CSS files? See the admin section and tutorial on http://metro-webdesign.info before enabling this!
$enableCompressionJs = false; // compress and combine Javascript files? See the admin section and tutorial on http://metro-webdesign.info before enabling this!
?>
